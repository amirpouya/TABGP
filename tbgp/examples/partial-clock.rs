   ...
   // let nfa = NFA::alt_nfa_gen(256, pattern_size, true, false);
    ...
    while c <  actives.len() || current_active.len() > 0 {
        ...
        if current_time < a.time {
            ...
            if pattern_size > 2 || pattern_type == "path2" {
                if !dec_flag {
                    path2_matching_forward = hash_join(&matching_tail, &edge_head)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, de)): (usize, &(PMatching, _, Edge))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array(de.eid, (e.tail_idx + 1) as usize as i8, e.eid), first: e.first, last: de.first, match_size: 2, head: e.head, head_idx: e.head_idx, tail: de.dst, tail_idx: if e.tail_idx + 1 >= (pattern_size - 1) as i8 { -1 } else { e.tail_idx + 1 }, state: e.state, word: 0 ,clocks: e.clocks
                        })
                        .collect_vec();
            ...
            if pattern_size > 3 || pattern_type == "path3" {
                ...
                if !dec_flag {
                    path3_matching_forward = hash_join(&matching_tail_2.clone().collect_vec(), &edge_head)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, de)): (usize, &(PMatching, _, Edge))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array(de.eid, e.tail_idx + 1, e.eid), first: e.first, last: de.first, match_size: 3, head: e.head, head_idx: e.head_idx, tail: de.dst, tail_idx: if e.tail_idx + 1 > (pattern_size - 1) as i8 { 0 } else { e.tail_idx + 1 }, state: e.state, word: 0,clocks: e.clocks })
                        .collect_vec();
                    ...
                    path1_path2_matching_forward = hash_join(&ee, &a)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, (de1, de2))): (usize, &(PMatching, _, (Edge, Edge)))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array_with_vec(vec![de1.eid, de2.eid], (e.tail_idx+1 ) as usize, e.eid), first: e.first, last: de1.first, match_size: 3, head: e.head, head_idx: e.head_idx, tail:de2.dst, tail_idx:  if e.tail_idx + 3 < pattern_size as i8 { e.tail_idx + 2} else {-1}, state: e.state, word: 0, clocks: e.clocks})
                        .collect_vec();
                ...
                if !inc_flag {
                    ...
                    path3_matching_reverse = hash_join(&matching_head_2.clone().collect_vec(), &edge_tail.clone())
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, de)): (usize, &(PMatching, _, Edge))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array(de.eid, e.head_idx-1, e.eid), first: e.first, last: de.first, match_size: 3, head: de.src, head_idx: if e.head_idx - 1 > 0 { e.head_idx-1 } else { -1 }, tail: e.tail, tail_idx: e.tail_idx, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
                    ...
                    path2_path1_matching_forward = hash_join(&ee, &a)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, (de1, de2))): (usize, &(PMatching, _, (Edge, Edge)))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array_with_vec(vec![de1.eid, de2.eid], (e.head_idx - 2) as usize, e.eid), first: e.first, last: de1.first, match_size: 3, head: de1.src, head_idx:if e.head_idx - 2 > 0 { e.head_idx-2} else { -1 }, tail: e.tail, tail_idx: e.tail_idx, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
            ...
            if pattern_size > 4 || pattern_type == "path4" {
                ...
                if !dec_flag {
                    path4_matching_forward = hash_join(&matching_tail_3.clone().collect_vec(), &edge_head)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, de)): (usize, &(PMatching, _, Edge))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array(de.eid, e.tail_idx + 1, e.eid), first: e.first, last: de.first, match_size: 4, head: e.head, head_idx: e.head_idx, tail: de.dst, tail_idx: if e.tail_idx + 1 > (pattern_size - 1) as i8 { 0 } else { e.tail_idx + 1 }, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
                    ...
                    path2_path2_matching_forward = hash_join(&eee, &a)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, (de1, de2))): (usize, &(PMatching, _, (Edge, Edge)))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array_with_vec(vec![de1.eid, de2.eid], (e.tail_idx + 1) as usize, e.eid), first: e.first, last: de1.first, match_size: 4, head: e.head, head_idx: e.head_idx, tail:de2.dst, tail_idx:  if e.tail_idx + 3 < pattern_size as i8 { e.tail_idx + 2} else {-1}, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
                    ...
                    path3_path1_matching_forward = hash_join(&eee, &a)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, (de1,de2, de3))): (usize, &(PMatching, _, (Edge, Edge, Edge)))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array_with_vec(vec![de1.eid, de2.eid, de3.eid], (e.tail_idx + 1) as usize, e.eid), first: e.first, last: de1.first, match_size: 4, head: e.head, head_idx: e.head_idx, tail:de2.dst, tail_idx:  if e.tail_idx + 4 < pattern_size as i8 { e.tail_idx + 3} else {-1}, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
                ...
                if !inc_flag {
                    ...
                    path4_matching_reverse = hash_join(&matching_head_3.clone().collect_vec(), &edge_tail.clone())
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, de)): (usize, &(PMatching, _, Edge))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array(de.eid, e.head_idx-1, e.eid), first: e.first, last: de.first, match_size: 4, head: de.src, head_idx: if e.head_idx - 1 > 0 { e.head_idx-1 } else { -1 }, tail: e.tail, tail_idx: e.tail_idx, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
                    ...
                    path2_path2_matching_reverse = hash_join(&eee, &a)
                        .iter()
                        .enumerate()
                        .map(|(i, (e, _, (de1, de2))): (usize, &(PMatching, _, (Edge, Edge)))| PMatching { mid: i + 1 + mid_counter, eid: PMatching::fill_array_with_vec(vec![de1.eid, de2.eid], (e.head_idx - 2) as usize, e.eid), first: e.first, last: de1.first, match_size: 4, head: de1.src, head_idx:if e.head_idx - 2 > 0 { e.head_idx-2} else { -1 }, tail: e.tail, tail_idx: e.tail_idx, state: e.state, word: 0, clocks: e.clocks })
                        .collect_vec();
            ...
            matching = TNFA::apply_partial_nfa(current_time,&nfa_join, &matching);
